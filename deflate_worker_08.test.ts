/**
 * Worker file 08/20 — replicates the structure of cross-org/image test files
...
for (let j = 0; j < PAYLOAD.length; j++) PAYLOAD[j] = (j * 8) & 0xff;

test("worker-08: deflate + inflate roundtrip", async () => {
...
test("worker-08: deflate compress small payload", async () => {
  const data = new Uint8Array([8, 16, 24, 255]);
  const compressed = await deflateCompress(data);
  const decompressed = await deflateDecompress(compressed);
  assertEquals(decompressed, data);
});

test("worker-08: deflate roundtrip partial payload", async () => {
