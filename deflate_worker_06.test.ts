/**
 * Worker file 06/20 — replicates the structure of cross-org/image test files
...
const PAYLOAD = new Uint8Array(1536);
for (let j = 0; j < PAYLOAD.length; j++) PAYLOAD[j] = (j * 6) & 0xff;

test("worker-06: deflate + inflate roundtrip", async () => {
...
test("worker-06: deflate compress small payload", async () => {
  const data = new Uint8Array([6, 12, 18, 255]);
  const compressed = await deflateCompress(data);
  const decompressed = await deflateDecompress(compressed);
  assertEquals(decompressed, data);
});

test("worker-06: deflate roundtrip partial payload", async () => {
