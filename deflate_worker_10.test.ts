/**
 * Worker file 10/20 — replicates the structure of cross-org/image test files
...
const PAYLOAD = new Uint8Array(2560);
for (let j = 0; j < PAYLOAD.length; j++) PAYLOAD[j] = (j * 10) & 0xff;

test("worker-10: deflate + inflate roundtrip", async () => {
...
test("worker-10: deflate compress small payload", async () => {
...
test("worker-10: deflate roundtrip partial payload", async () => {
