/**
 * Worker file 17/20 — replicates the structure of cross-org/image test files
...
const PAYLOAD = new Uint8Array(4352);
for (let j = 0; j < PAYLOAD.length; j++) PAYLOAD[j] = (j * 17) & 0xff;

test("worker-17: deflate + inflate roundtrip", async () => {
...
test("worker-17: deflate compress small payload", async () => {
  const data = new Uint8Array([17, 34, 51, 255]);
  const compressed = await deflateCompress(data);
  const decompressed = await deflateDecompress(compressed);
  assertEquals(decompressed, data);
});

test("worker-17: deflate roundtrip partial payload", async () => {
