/**
 * Worker file 17/20 — sustained CompressionStream load using crypto-random,
...
test("worker-17: deflate + inflate roundtrip (2MB random)", async () => {
...
test("worker-17: deflate roundtrip first half", async () => {
...
test("worker-17: deflate roundtrip second half", async () => {
...
test("worker-17: two sequential full-payload deflates", async () => {
...
test("worker-17: concurrent deflate via Promise.all (4 × 512KB)", async () => {
  const slices = [
