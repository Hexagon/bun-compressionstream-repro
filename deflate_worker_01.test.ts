/**
 * Worker file 01/20 — replicates the structure of cross-org/image test files
...
for (let j = 0; j < PAYLOAD.length; j++) PAYLOAD[j] = (j * 1) & 0xff;

test("worker-01: deflate + inflate roundtrip", async () => {
...
test("worker-01: deflate compress small payload", async () => {
...
test("worker-01: deflate roundtrip partial payload", async () => {
