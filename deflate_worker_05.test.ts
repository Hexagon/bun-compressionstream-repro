/**
 * Worker file 05/20 — replicates the structure of cross-org/image test files
...
const PAYLOAD = new Uint8Array(1280);
for (let j = 0; j < PAYLOAD.length; j++) PAYLOAD[j] = (j * 5) & 0xff;

test("worker-05: deflate + inflate roundtrip", async () => {
...
test("worker-05: deflate compress small payload", async () => {
  const data = new Uint8Array([5, 10, 15, 255]);
  const compressed = await deflateCompress(data);
  const decompressed = await deflateDecompress(compressed);
  assertEquals(decompressed, data);
});

test("worker-05: deflate roundtrip partial payload", async () => {
