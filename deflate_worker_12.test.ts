/**
 * Worker file 12/20 — replicates the structure of cross-org/image test files
...
for (let j = 0; j < PAYLOAD.length; j++) PAYLOAD[j] = (j * 12) & 0xff;

test("worker-12: deflate + inflate roundtrip", async () => {
...
test("worker-12: deflate compress small payload", async () => {
  const data = new Uint8Array([12, 24, 36, 255]);
  const compressed = await deflateCompress(data);
  const decompressed = await deflateDecompress(compressed);
  assertEquals(decompressed, data);
});

test("worker-12: deflate roundtrip partial payload", async () => {
