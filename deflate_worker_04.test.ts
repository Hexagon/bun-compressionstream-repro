/**
 * Worker file 04/20 — replicates the structure of cross-org/image test files
...
for (let j = 0; j < PAYLOAD.length; j++) PAYLOAD[j] = (j * 4) & 0xff;

test("worker-04: deflate + inflate roundtrip", async () => {
...
test("worker-04: deflate compress small payload", async () => {
  const data = new Uint8Array([4, 8, 12, 255]);
  const compressed = await deflateCompress(data);
  const decompressed = await deflateDecompress(compressed);
  assertEquals(decompressed, data);
});

test("worker-04: deflate roundtrip partial payload", async () => {
