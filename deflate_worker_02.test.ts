/**
 * Worker file 02/20 — replicates the structure of cross-org/image test files
...
for (let j = 0; j < PAYLOAD.length; j++) PAYLOAD[j] = (j * 2) & 0xff;

test("worker-02: deflate + inflate roundtrip", async () => {
...
test("worker-02: deflate compress small payload", async () => {
  const data = new Uint8Array([2, 4, 6, 255]);
  const compressed = await deflateCompress(data);
  const decompressed = await deflateDecompress(compressed);
  assertEquals(decompressed, data);
});

test("worker-02: deflate roundtrip partial payload", async () => {
