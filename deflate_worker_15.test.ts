/**
 * Worker file 15/20 — replicates the structure of cross-org/image test files
...
const PAYLOAD = new Uint8Array(3840);
for (let j = 0; j < PAYLOAD.length; j++) PAYLOAD[j] = (j * 15) & 0xff;

test("worker-15: deflate + inflate roundtrip", async () => {
...
test("worker-15: deflate compress small payload", async () => {
  const data = new Uint8Array([15, 30, 45, 255]);
  const compressed = await deflateCompress(data);
  const decompressed = await deflateDecompress(compressed);
  assertEquals(decompressed, data);
});

test("worker-15: deflate roundtrip partial payload", async () => {
