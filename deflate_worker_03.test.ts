/**
 * Worker file 03/20 — replicates the structure of cross-org/image test files
...
for (let j = 0; j < PAYLOAD.length; j++) PAYLOAD[j] = (j * 3) & 0xff;

test("worker-03: deflate + inflate roundtrip", async () => {
...
test("worker-03: deflate compress small payload", async () => {
  const data = new Uint8Array([3, 6, 9, 255]);
  const compressed = await deflateCompress(data);
  const decompressed = await deflateDecompress(compressed);
  assertEquals(decompressed, data);
});

test("worker-03: deflate roundtrip partial payload", async () => {
