/**
 * Worker file 11/20 — replicates the structure of cross-org/image test files
...
const PAYLOAD = new Uint8Array(2816);
for (let j = 0; j < PAYLOAD.length; j++) PAYLOAD[j] = (j * 11) & 0xff;

test("worker-11: deflate + inflate roundtrip", async () => {
...
test("worker-11: deflate compress small payload", async () => {
  const data = new Uint8Array([11, 22, 33, 255]);
  const compressed = await deflateCompress(data);
  const decompressed = await deflateDecompress(compressed);
  assertEquals(decompressed, data);
});

test("worker-11: deflate roundtrip partial payload", async () => {
