/**
 * Worker file 19/20 — replicates the structure of cross-org/image test files
...
for (let j = 0; j < PAYLOAD.length; j++) PAYLOAD[j] = (j * 19) & 0xff;

test("worker-19: deflate + inflate roundtrip", async () => {
...
test("worker-19: deflate compress small payload", async () => {
  const data = new Uint8Array([19, 38, 57, 255]);
  const compressed = await deflateCompress(data);
  const decompressed = await deflateDecompress(compressed);
  assertEquals(decompressed, data);
});

test("worker-19: deflate roundtrip partial payload", async () => {
