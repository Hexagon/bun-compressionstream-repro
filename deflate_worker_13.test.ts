/**
 * Worker file 13/20 — replicates the structure of cross-org/image test files
...
const PAYLOAD = new Uint8Array(3328);
for (let j = 0; j < PAYLOAD.length; j++) PAYLOAD[j] = (j * 13) & 0xff;

test("worker-13: deflate + inflate roundtrip", async () => {
...
test("worker-13: deflate compress small payload", async () => {
  const data = new Uint8Array([13, 26, 39, 255]);
  const compressed = await deflateCompress(data);
  const decompressed = await deflateDecompress(compressed);
  assertEquals(decompressed, data);
});

test("worker-13: deflate roundtrip partial payload", async () => {
