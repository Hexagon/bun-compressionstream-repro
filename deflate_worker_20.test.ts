/**
 * Worker file 20/20 — sustained CompressionStream load using crypto-random,
...
test("worker-20: deflate + inflate roundtrip (2MB random)", async () => {
...
test("worker-20: deflate roundtrip first half", async () => {
...
test("worker-20: deflate roundtrip second half", async () => {
...
test("worker-20: two sequential full-payload deflates", async () => {
...
test("worker-20: concurrent deflate via Promise.all (4 × 512KB)", async () => {
  const slices = [
