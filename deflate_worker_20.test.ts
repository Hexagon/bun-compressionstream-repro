/**
 * Worker file 20/20 — replicates the structure of cross-org/image test files
...
const PAYLOAD = new Uint8Array(5120);
for (let j = 0; j < PAYLOAD.length; j++) PAYLOAD[j] = (j * 20) & 0xff;

test("worker-20: deflate + inflate roundtrip", async () => {
...
test("worker-20: deflate compress small payload", async () => {
  const data = new Uint8Array([20, 40, 60, 255]);
  const compressed = await deflateCompress(data);
  const decompressed = await deflateDecompress(compressed);
  assertEquals(decompressed, data);
});

test("worker-20: deflate roundtrip partial payload", async () => {
