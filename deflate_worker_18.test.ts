/**
 * Worker file 18/20 — replicates the structure of cross-org/image test files
...
const PAYLOAD = new Uint8Array(4608);
for (let j = 0; j < PAYLOAD.length; j++) PAYLOAD[j] = (j * 18) & 0xff;

test("worker-18: deflate + inflate roundtrip", async () => {
...
test("worker-18: deflate compress small payload", async () => {
  const data = new Uint8Array([18, 36, 54, 255]);
  const compressed = await deflateCompress(data);
  const decompressed = await deflateDecompress(compressed);
  assertEquals(decompressed, data);
});

test("worker-18: deflate roundtrip partial payload", async () => {
