/**
 * Worker file 18/20 — sustained CompressionStream load using crypto-random,
...
test("worker-18: deflate + inflate roundtrip (2MB random)", async () => {
...
test("worker-18: deflate roundtrip first half", async () => {
...
test("worker-18: deflate roundtrip second half", async () => {
...
test("worker-18: two sequential full-payload deflates", async () => {
...
test("worker-18: concurrent deflate via Promise.all (4 × 512KB)", async () => {
  const slices = [
