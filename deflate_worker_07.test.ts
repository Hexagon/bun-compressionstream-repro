/**
 * Worker file 07/20 — replicates the structure of cross-org/image test files
...
const PAYLOAD = new Uint8Array(1792);
for (let j = 0; j < PAYLOAD.length; j++) PAYLOAD[j] = (j * 7) & 0xff;

test("worker-07: deflate + inflate roundtrip", async () => {
...
test("worker-07: deflate compress small payload", async () => {
  const data = new Uint8Array([7, 14, 21, 255]);
  const compressed = await deflateCompress(data);
  const decompressed = await deflateDecompress(compressed);
  assertEquals(decompressed, data);
});

test("worker-07: deflate roundtrip partial payload", async () => {
