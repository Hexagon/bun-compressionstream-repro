/**
 * Worker file 16/20 — heavy CompressionStream load to reproduce the
...
const PAYLOAD = (() => {
  ...
  for (let j = 0; j < buf.length; j++) buf[j] = (j * 16 + j) & 0xff;
  return buf;
})();

test("worker-16: deflate + inflate roundtrip A", async () => {
...
test("worker-16: deflate + inflate roundtrip B", async () => {
...
test("worker-16: deflate + inflate roundtrip C", async () => {
...
test("worker-16: three sequential deflate calls", async () => {
...
test("worker-16: concurrent deflate via Promise.all", async () => {
  const slices = [
