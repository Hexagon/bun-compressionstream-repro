/**
 * Worker file 14/20 — replicates the structure of cross-org/image test files
...
const PAYLOAD = new Uint8Array(3584);
for (let j = 0; j < PAYLOAD.length; j++) PAYLOAD[j] = (j * 14) & 0xff;

test("worker-14: deflate + inflate roundtrip", async () => {
...
test("worker-14: deflate compress small payload", async () => {
  const data = new Uint8Array([14, 28, 42, 255]);
  const compressed = await deflateCompress(data);
  const decompressed = await deflateDecompress(compressed);
  assertEquals(decompressed, data);
});

test("worker-14: deflate roundtrip partial payload", async () => {
